...
for i in range(50):
    ...
    dataloader = DataLoader(idxs[fake_train_mask], batch_size=128,
    ...
    for epoch in range(5):
        for batch,batch_walk in enumerate(dataloader):
            # there might be -1 in batch_walk
            if batch_walk.min()==-1:
    ...
    idxs=torch.arange(graph.num_nodes()-2)
    dataloader = DataLoader(idxs[train_mask], batch_size=128,
                            shuffle=True, collate_fn=model.sample,drop_last=True)
    frozen_label=origin_rep[-2:].detach().requires_grad_(False)
    # print(model2.node_embed.weight[-1])
    # print(model2.node_embed.weight[-2])

    # model2.node_embed.weight[-1].detach().requires_grad_(False)
    # model2.node_embed.weight[-1]=frozen_label[-1]
    # model2.node_embed.weight[-2].detach().requires_grad_(False)
    ...
    for epoch in range(EPOCH):
        ...
        if epoch%interval==0:
            acc=evaluate(model2,fake_node_labels,valid_mask)
            print(f'epoch {epoch}, loss {loss.item():.4f}, valid acc {acc:.4f}')
